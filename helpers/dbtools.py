...
async def handle_user_status(bot, cmd):
    ...
    if not await db.is_user_exist(chat_id):
        ...
        await bot.send_message(
            LOG_CHANNEL,
            f"**📣 Notifikasi Bot.** \n\n#PENGGUNA_BARU **Mulai menggunakan Bot anda!** \n\n🏷 nama: `{cmd.from_user.first_name}` \n📮 id pengguna: `{cmd.from_user.id}` \n🧝🏻‍♂️ profil: [{cmd.from_user.first_name}](tg://user?id={cmd.from_user.id})",
        )
    ...
    if ban_status["is_banned"]:
        if (
            ...
            await cmd.reply_text(
                f"maaf, Anda diblokir, tanyakan di @{GROUP_SUPPORT} jika menurut Anda ini adalah kesalahan.",
                quote=True,
            )
            return
...
async def main_broadcast_handler(m, db):
    ...
    out = await m.reply_text(
        text="**💡 Mulai menyiarkan...**\n\n**» setelah selesai, Anda akan diberi tahu di sini !**"
    )
    ...
    if failed == 0:
        await m.reply_text(
            text=f"✅ selesai menyiarkan! \n**selesai pada:** `{completed_in}` \n\n**Total pengguna:** `{total_users}` \n**Total selesai:** `{done}` \n**Total sukses:** `{success}` \n**Total gagal:** `{failed}`",
            quote=True,
        )
    else:
        await m.reply_document(
            document="broadcast-logs.txt",
            caption=f"✅ selesai menyiarkan! \n**selesai pada:** `{completed_in}` \n\n**Total pengguna:** `{total_users}` \n**Total selesai:** `{done}` \n**Total sukses:** `{success}` \n**Total gagal:** `{failed}`",
            quote=True,
        )
