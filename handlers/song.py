...
def song(_, message):
    ...
    m = message.reply("**🔎 cie ketauan galau..** \n\n gabut apa galau nih...")
    ...
    try:
        ...
        m.edit("❌ Lagu tidak ditemukan.\n\nMohon berikan judul lagu yang benar.")
        print(str(e))
        return
    m.edit("📥 nih gw unduhin lagu \n\n buat yang lagi galau...")
    try:
        with yt_dlp.YoutubeDL(ydl_ops) as ydl:
            info_dict = ydl.extract_info(link, download=False)
            audio_file = ydl.prepare_filename(info_dict)
            ydl.process_info(info_dict)
        rep = f"**🎧 Uploader 𝓚𝓲𝓶.𝓲𝓭**"
...
async def vsong(client, message):
    ...
    try: 
        ...
        print(e)
    try:
        msg = await message.reply("📥 **Mendownload video...**")
...
async def lyrics(_, message):
    try:
        if len(message.command) < 2:
            await message.reply_text("» **berikan judul lirik juga .**")
            return
        ...
        await rep.edit("❌ **lirik tidak ditemukan.**\n\n» **Mohon berikan judul lagu dengan benar.**")
